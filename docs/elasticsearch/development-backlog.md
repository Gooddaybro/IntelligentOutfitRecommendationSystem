...
| P2 | Outbox + RabbitMQ 增量同步 | 设计完成 | 商品事务提交后可靠、幂等地更新对应 SPU 文档，全量重建继续作为修复手段 |
...
- ?? `product_search_outbox` / `product_search_inbox` ???????????
- ???????????????????????????????????
- ?? Product Search RabbitMQ ??????????????? DLQ?
- Publisher Confirm?Return???????????
- Worker ???????Inbox ????????????????? DLQ?
- ???? W0/W1 ?????

P2 ????? Web/Worker/MySQL/RabbitMQ/ES ?????P3 ?????? P4 ???????????

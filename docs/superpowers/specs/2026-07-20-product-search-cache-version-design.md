...
```text
product:search:v{generation}:{normalizedKeyword}:{normalizedCategory}
```
...
```text
product:search:v{version}:{keyword}:{category}
```

该组件不读取数据库或 Redis，使键格式能够单独测试，避免版本逻辑散落在 `ProductCatalogService`。

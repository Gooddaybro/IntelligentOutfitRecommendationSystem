...
2. 在 Python → Java 结果中加入 `recommendation_status`；保留 `recommended_items` 作为唯一商品归因清单，并为穿搭建议中的每个推荐项增加可选 `outfit_role`。
...
- Java `AssistantRecommendationItem`、Python 响应 DTO、Validator 与 SSE DTO
...
1. Python 只能为实际选中的 Java 候选附加 `outfit_role`，并根据商品分类和穿搭方案分配角色。
2. Java 校验 `outfit_role` 是否属于允许枚举，同时继续校验 SPU/SKU 必须来自本轮候选池；非法角色降级为无分组推荐项，不影响整次回答。
3. SSE 将角色随 `recommended_items` 传给前端，旧版本没有角色时保持当前平铺展示，保证兼容升级。
...
6. 个人资料表单可读性和控件状态修复。
7. 本轮身体数据的一键保存与资料同步。
8. 穿搭角色字段贯穿 Python、Java、SSE 和前端分组展示。
9. 商品标签补齐、端到端测试和回归测试。
...
3. **身体数据：** 采用方案 C。对话中识别出的身高体重立即作为本轮临时上下文使用，但只有用户明确点击“保存为我的身体数据”后才写入个人资料。资料中数值相同时不提示；数值不同时展示新旧值并二次确认；保存失败不影响当前穿搭回答和推荐结果。
4. **穿搭商品：** 采用方案 B。按 `TOP`、`BOTTOM`、`OUTER`、`SHOES`、`ACCESSORY` 分组展示真实候选商品，缺失角色只给文字建议，购买行为仍以单个 SKU 为单位。

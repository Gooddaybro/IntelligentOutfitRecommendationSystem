...
- [ ] 地址、订单、订单项、地址快照、库存变化、购物车清理、订单事件和幂等业务结果位于同一事务；严格事件写入修复已在工作区完成并通过针对性测试，仍待全量验证、复审和提交。
...
- [ ] 前端每次用户主动提交生成 UUID `Idempotency-Key`；购物车下单已提交完成，立即购买修复及测试已在工作区完成，仍待前端全量验证、复审和提交。
- [ ] 同一次网络重试复用原键；用户修改地址或商品选择后生成新键。购物车下单已验证，立即购买仍待最终验收。
...
- [x] 增加订单严格事件写入入口，使订单事件失败能够触发整笔交易回滚；针对性单测和真实 MySQL 故障注入测试已通过。
- [x] 正式下单读取购物车事实前锁定所选购物车行；针对性单测和真实 MySQL 并发测试已通过。
- [x] 立即购买请求增加 UUID `Idempotency-Key`，失败重试复用同一键，商品或数量变化后换键；实现和测试代码已保留在工作区。
- [x] 结算页直接展示服务端 `lineAmount`，不再由浏览器重复计算行金额；实现和测试代码已保留在工作区。
...
- [ ] 运行后端阶段 4 针对性测试。
- [ ] 运行真实 MySQL 回滚与并发测试。
- [ ] 运行前端全量测试和生产构建，确认立即购买幂等键与 `lineAmount` 改动。
- [ ] 运行 Java 全量 `mvn verify`。
- [ ] 对最终复审修复做一次独立复审，确认没有 Critical/Important 问题。
- [ ] 将当前工作区修复提交为独立提交；在上述门禁全部通过前，不把阶段 4 标记为最终完成。

**当前结论：** 阶段 4 主体功能已完成并提交，最终复审收口暂停在“代码已修改、最终验收和提交未完成”。下一次无需重做 Task 4.1～4.3，应直接从上述测试、复审和提交步骤继续。

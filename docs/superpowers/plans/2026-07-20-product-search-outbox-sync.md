...
- [ ] 先写持久化测试，断言 Outbox 唯一键、发布扫描索引语义和 Inbox 去重。
- [ ] 运行测试，确认表和 Mapper 缺失。
- [ ] 创建独立 Outbox/Inbox 表和默认关闭、带合法范围校验的配置。
- [ ] 运行数据库迁移与配置测试，中文提交。
...
- [ ] 先写失败测试，定义 `record(spuId)`、最大水位和区间去重 SPU 查询契约。
- [ ] 实现 schemaVersion=1 的最小消息和同事务 Outbox INSERT。
- [ ] 使用条件 Bean 在总开关关闭时提供 No-op Recorder。
- [ ] 验证序列化、唯一事件 ID、水位查询和事务回滚，中文提交。
...
- [ ] 先写测试，证明创建、编辑、状态修改记录 SPU，分类修改记录全部受影响 SPU，库存调整不记录。
- [ ] 注入 Recorder，在业务 SQL 成功后、事务返回前记录事件。
- [ ] 分类更新前读取受影响 SPU ID，并在同一事务逐个记录。
- [ ] 运行 Admin 服务和事务测试，中文提交。
...
- [ ] 先写拓扑和 Relay 失败测试，覆盖租约竞争、Confirm ACK、NACK、Return 与超时。
- [ ] 声明主队列、10/60/300 秒延迟队列和 DLQ。
- [ ] Relay 只在 web profile、总开关和 publisher 开关开启时工作。
- [ ] 只有 Confirm ACK 且没有 Return 才标记 PUBLISHED，中文提交。
...
- [ ] 先写测试，覆盖最新事实 UPSERT、物理缺失 DELETE、重复事件、乱序事件、分级重试和 DLQ。
- [ ] 增加按 SPU ID 读取完整索引投影的 Mapper 查询。
- [ ] Projector 使用 `product_current` 和固定 SPU `_id` 执行幂等操作。
- [ ] Worker 在 ES 成功后写 Inbox，发布重试/DLQ 成功后才 ACK 原消息。
- [ ] 运行 Worker、投影和 Mapper 测试，中文提交。
...
- [ ] 先写测试，证明 W0/W1 区间去重、切换后补偿和同步关闭时跳过水位逻辑。
- [ ] 重建开始读取 W0，别名切换后读取 W1，并投影 `(W0, W1]` 的不同 SPU。
- [ ] 补偿失败返回明确错误，不删除已经成为当前别名目标的新索引。
- [ ] 运行重建、生命周期和并发补偿测试，中文提交。
...
- [ ] 启动 MySQL、RabbitMQ、Elasticsearch，并验证独立拓扑。
- [ ] 修改商品，验证 Outbox、RabbitMQ、Inbox 和 ES 最终状态。
- [ ] 验证重复消息、ES 停止恢复和重建水位补偿。
- [ ] 更新配置、运维、DLQ 和已知 Redis 缓存窗口说明。
- [ ] 运行 `backend\\mvnw.cmd verify`、Checkstyle 和 `git diff --check`，中文提交。

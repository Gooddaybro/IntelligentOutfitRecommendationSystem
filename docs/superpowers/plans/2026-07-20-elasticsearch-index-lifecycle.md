...
- [ ] 先增加测试，断言默认保留两个历史索引且负数配置启动失败。
- [ ] 运行测试，确认因缺少属性或校验而失败。
- [ ] 增加 `retainedHistoryCount` 和 `@Min(0)` 校验，环境变量为 `APP_ELASTICSEARCH_RETAINED_HISTORY_COUNT`。
- [ ] 运行配置测试并提交中文 Git 提交。
...
- [ ] 先增加测试，覆盖当前索引保护、最近两个历史索引保留、严格 14 位时间戳和异常名称排除。
- [ ] 运行测试，确认新类型不存在。
- [ ] 实现纯 Java 保留策略，按创建时间降序选择删除集合，并写明双重删除保护原因。
- [ ] 运行策略测试并提交中文 Git 提交。
...
- [ ] 先增加测试，覆盖失败索引幂等删除、别名目标读取、超限历史索引删除和非受管索引保护。
- [ ] 运行测试，确认服务不存在。
- [ ] 使用官方 Client 实现索引查询和删除；任何删除前再次应用严格保留策略。
- [ ] 运行生命周期服务测试并提交中文 Git 提交。
...
- [ ] 先增加编排测试：Bulk/数量校验失败只清理本次索引；创建失败不删除；失败清理不覆盖原异常；成功清理失败仍返回成功。
- [ ] 运行测试，确认现有重建服务不满足行为。
- [ ] 注入生命周期服务，记录索引是否已创建，并在 catch/finally 边界执行 best-effort 清理。
- [ ] 运行重建服务测试并提交中文 Git 提交。
...
- [ ] 连续执行四次真实重建，确认别名只有一个目标且受管索引最多三个。
- [ ] 验证文档数量和中文搜索结果。
- [ ] 更新配置、清理行为、人工回滚说明和 P1 状态。
- [ ] 运行 `backend\\mvnw.cmd verify` 与 `git diff --check`。
- [ ] 使用中文 Git 提交完成文档和验证闭环。

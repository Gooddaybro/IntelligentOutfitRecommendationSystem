...
class ProAssistantServiceTests {
    ...
    private ProAssistantService service(boolean enabled) {
        return new ProAssistantService(conversations, limiter, context, registry, client, validator, enabled);
    }

    private void ready() throws Exception {
        when(validator.validate(eq(7L), eq("th-1"), anyString(), eq(run), eq(request), any())).thenReturn(new ObjectMapper().readTree("{\"answer\":\"validated\"}"));
    ...
    void ownershipThenUserPersistenceThenContextAndV2CallThenRevoke() throws Exception {
        ...
        assertThat(result.path("answer").asText()).isEqualTo("validated");
        var order = inOrder(conversations, registry, context, client, validator);
        ...
        order.verify(validator).validate(eq(7L), eq("th-1"), anyString(), eq(run), eq(request), any());

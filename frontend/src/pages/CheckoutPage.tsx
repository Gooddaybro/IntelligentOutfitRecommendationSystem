...
export function CheckoutPage({ onOrderCreated }: { onOrderCreated: (order: OrderResponse) => void }) {
  ...
  return <main className="checkout-page">
    ...
    <div className="checkout-grid"><div>
      ...
      <section className="checkout-card"><h2><PackageCheck size={20}/>商品清单</h2>{preview?.items.map((item) => <article className="checkout-item" key={item.skuId}><span>{item.name}<small>{item.color} · {item.size} × {item.quantity}</small></span><strong>¥{item.lineAmount.toFixed(2)}</strong></article>)}</section>
